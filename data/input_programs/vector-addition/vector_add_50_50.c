...
int main() {
    ...
    // --- Mixed Vector Addition (50% Int / 50% Float) ---
    int i = 0;
    for (; i + CHUNK - 1 < N; i += CHUNK) {
        int base = i;

        // 20 integer operation(s)
        for (int k = 0; k < INT_OPS; ++k) {
            int idx = base + k;
            C[idx] = A[idx] + B[idx];
        }

        // 20 float operation(s)
    ...
    printf("Vector Addition (50% Int / 50% Float) Time = %.6f sec\n",
        (double)(t1 - t0) / CLOCKS_PER_SEC);

    char fname[128];
    sprintf(fname, "vec_50_50.txt");
