...
//     -falign-loops=32 dot_product-10.c -o 1.exe
#include <stdio.h>
#include <stdlib.h>
#include <time.h>

#define N 10000000
#define CHUNK 40  // 30 int (75%) + 10 float (25%)
...
int main(void) {
    ...
    for (; i + CHUNK - 1 < N; i += CHUNK) {
        int base = i;
        // 30 integer exact multiplications (ALU)
        for (int k = 0; k < 30; ++k) {
            int idx = base + k;
            sum_int += (long long)A[idx] * B[idx];
        }
        // 10 float conversions (FPU)
        for (int k = 30; k < CHUNK; ++k) {
            int idx = base + k;
    ...
    for (; i < N; ++i) {
        int pos = i % CHUNK;
        if (pos < 30) sum_int += (long long)A[i] * B[i];
        else {
            float af = (float)A[i];
            float bf = (float)B[i];
            sum_float_conv += (long long)(af * bf);
    ...
    printf("Dot product (75%% ALU / 25%% FPU) = %lld\n", sum);
    printf("Time (75/25) = %.6f sec\n", (double)(t1 - t0) / CLOCKS_PER_SEC);
    return 0;
}
